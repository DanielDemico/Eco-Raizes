...
import PersonalizacaoVasos from './PersonalizacaoVasos';
...
const StoreCarousel: React.FC<StoreCarouselProps> = ({ whatsappNumber, whatsappMessage }) => {
  ...
  return (
      ...
      <section id="loja" className="py-16 sm:py-24 bg-white relative overflow-hidden">
        <div className="container mx-auto px-12">
          ...
          <div className="relative">
            ...
            <button
              ...
              className={`hidden md:flex items-center justify-center absolute -left-12 top-1/2 -translate-y-1/2 z-10 bg-white shadow-md rounded-full p-2 hover:bg-eco-green hover:text-white transition disabled:opacity-30 ${pressed === 'left' ? 'scale-90' : ''}`}
              disabled={current === 0}
            >
            ...
            <button
              ...
              className={`hidden md:flex items-center justify-center absolute -right-12 top-1/2 -translate-y-1/2 z-10 bg-white shadow-md rounded-full p-2 hover:bg-eco-green hover:text-white transition disabled:opacity-30 ${pressed === 'right' ? 'scale-90' : ''}`}
              ...
              {STORE_CATEGORIES.map((category: Category, idx: number) => (
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <a href={`https://wa.me/${whatsappNumber.replace(/\D/g, '')}?text=${encodeURIComponent(`Olá! Gostaria de saber mais sobre a categoria: ${category.name}`)}`}
                      target="_blank"
            ...
            <div className="flex justify-center mt-4 gap-2">
              {STORE_CATEGORIES.map((_, idx) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full border border-eco-green transition-colors duration-300 ${idx === current ? 'bg-eco-green' : 'bg-white'}`}
